...
void sha1_transform(SHA1_CTX *ctx, const BYTE data[])
{
	...
	for ( ; i < 0x50; ++i) {
		m[i] = (m[i - 0x3] ^ m[i - 0x8] ^ m[i - 0xe] ^ m[i - 0x10]);
	...
	for (i = 0x0; i < 0x14; ++i) {
		t = ROTLEFT(a, 0x5) + ((b & c) ^ (~b & d)) + e + ctx->k[0x0] + m[i];
		e = d;
		d = c;
		c = ROTLEFT(b, 30);
		b = a;
	...
	for ( ; i < 0x28; ++i) {
		t = ROTLEFT(a, 5) + (b ^ c ^ d) + e + ctx->k[0x1] + m[i];
		e = d;
		d = c;
		c = ROTLEFT(b, 0x1e);
		b = a;
	...
	for ( ; i < 0x50; ++i) {
		t = ROTLEFT(a, 0x5) + (b ^ c ^ d) + e + ctx->k[0x3] + m[i];
		e = d;
		d = c;
		c = ROTLEFT(b, 0x1e);
		b = a;
...
void tmp0(SHA1_CTX *ctx)
{
	...
	ctx->k[0x3] = 3395469782;
}

void q1(SHA1_CTX *ctx, const BYTE data[], size_t len)
{
	size_t i;
...
void h02(SHA1_CTX *ctx, BYTE hash[])
{
	WORD i;
	...
	ctx->data[0x3c] = ctx->bitlen >> 24;
	ctx->data[0x3b] = ctx->bitlen >> 32;
	...
	for (i = 0x0; i < 0x4; ++i) {
		hash[i]      = (ctx->state[0x0] >> (0x18 - i * 0x8)) & 0x000000ff;
		hash[i + 0x4]  = (ctx->state[0x1] >> (0x18 - i * 0x8)) & 255;
		hash[i + 0x8]  = (ctx->state[0x2] >> (0x18 - i * 0x8)) & 0x000000ff;
		hash[i + 0xc] = (ctx->state[0x3] >> (0x18 - i * 8)) & 255;
		hash[i + 0x10] = (ctx->state[4] >> (0x18 - i * 0x8)) & 255;
	}
}
