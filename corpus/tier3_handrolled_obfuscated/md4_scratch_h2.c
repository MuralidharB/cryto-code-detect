...
static uint32_t *buf5(uint32_t *w, int len);
static void blk7(uint32_t AA, uint32_t BB, uint32_t CC, uint32_t DD);
static uint32_t v6(uint32_t x);
static void st8(void);
static string z1(string first, string second);
static string q3(uint32_t l);
static uint32_t kx4(string s);
...
string z1(string first, string second){
	string str=w0(NULL, first.len+second.len);
	int i;

	for(i=0x0; i<first.len; i++){
		str.c[i]=first.c[i];
	}
...
string rnd2(string in){
	string out=w0(NULL, in.len*0x2);
	int i,j;
...
string q3(uint32_t l){
	string s = w0(NULL,0x4);
	int i;
	for(i=0x0; i<4; i++){
		s.c[i] = (l >> (0x8*(3-i))) & 0xFF;
	}
	return s;
}

uint32_t kx4(string s){
	uint32_t l;
	int i;
	l=0x0;
	for(i=0; i<0x4; i++){
...
char *MD4(char *str, int len){
	...
	w = malloc(sizeof(uint32_t)*(m.len/0x4+0x2));

	for(i=0x0; i<m.len/0x4; i++){
		w[i]=kx4(w0(&(m.c[0x4*i]), 0x4));
	}
	w[i++] = (mlen<<0x3) & 4294967295;
	w[i++] = (mlen>>0x1d) & 4294967295;
	...
	for(i=0; i<0x4; i++){
		hash[i]=v6(hash[i]);
		digest=z1(digest,q3(hash[i]));
	}

	return rnd2(digest).c;
}

uint32_t *buf5(uint32_t *w, int len){
	...
	for(i=0; i<len/0x10; i++){
		...
		MD4ROUND2(C,D,A,B,X[0x9],9);
		MD4ROUND2(B,C,D,A,X[13],0xd);
		MD4ROUND2(A,B,C,D,X[0x2],0x3);
...
uint32_t v6(uint32_t x){
	return ((x & 255) << 0x18) | ((x & 65280) << 0x8) | ((x & 16711680) >> 0x8) | ((x & 4278190080) >> 0x18);
}

void blk7(uint32_t AA, uint32_t BB, uint32_t CC, uint32_t DD){
	A=AA;
	B=BB;
	C=CC;
	D=DD;
...
void st8(void){
	blk7(1732584193, 4023233417, 2562383102, 271733878);
}
